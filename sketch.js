...
function initializeDrones() {
  ...
  droneLastY = null;
  lastDroneFrame = -1;
  // Spawn ONE drone in center - clean and simple
  drones.push({
...
function keyPressed() {
  // Spacebar activates demo mode and loads/plays CASA DETROIT 2026 (casa.mp3)
  if (key === ' ' || keyCode === 32) {
    ...
    // Also handle audio loading/playing
    // Check if casa.mp3 is currently loaded
    const isCasaPlaying = audio && (currentAudioFile === 'casa.mp3' || audio.src.includes('casa.mp3'));
    
    if (isCasaPlaying) {
      // If casa.mp3 is already loaded, restart from beginning
      if (audio) {
...
function drawDrones() {
  ...
  // FORCE only one drone - remove all others immediately
  if (drones.length > 1) {
    drones = [drones[0]];
  }
  ...
  if (distFromCenter > maxRadius) {
    ...
    const angleToCenter = atan2(centerY - drone.y, centerX - drone.x);
    // Convert angle to nearest 90-degree direction
    if (angleToCenter > -PI/4 && angleToCenter <= PI/4) {
      drone.moveDirection = 0; // Right
    } else if (angleToCenter > PI/4 && angleToCenter <= 3*PI/4) {
      drone.moveDirection = 1; // Down
    } else if (angleToCenter > 3*PI/4 || angleToCenter <= -3*PI/4) {
      drone.moveDirection = 2; // Left
    } else {
  ...
  // Detect and prevent jumps (only for large unexpected jumps, not normal movement)
  if (droneLastX !== null && prevX !== undefined && isFinite(prevX) && prevY !== undefined && isFinite(prevY)) {
    const jumpDistance = dist(drone.x, drone.y, droneLastX, droneLastY || prevY);
    // Only prevent jumps > 100 pixels (normal movement is < 5 pixels per frame)
    if (jumpDistance > 100) {
      drone.x = droneLastX; // Revert to last known good position
      drone.y = droneLastY || prevY;
    } else {
      droneLastX = drone.x; // Update last known position
      droneLastY = drone.y;
    }
  } else {
    droneLastX = drone.x; // Initialize tracking
    droneLastY = drone.y;
  }
